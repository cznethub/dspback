...
class ZenodoRecord(BaseRecord):
    ...
    def to_submission(self, identifier) -> Submission:
        ...
        view_url = settings.zenodo_public_view_url % identifier if self.publication_date else settings.zenodo_view_url % identifier
        return Submission(
            title=self.title,
...
class EarthChemRecord(BaseRecord):
    ...
    def to_submission(self, identifier) -> Submission:
        settings = get_settings()
        view_url = settings.earthchem_public_view_url % identifier if self.datePublished else settings.earthchem_view_url % identifier
